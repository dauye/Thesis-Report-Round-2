...
\chapter{Conclusion}
    ...
    Additionally, the results from the building parameter variations indicate that the most influential parameters in simulation are key area temperature heating set points and external wall solar absorptance. Apart from them, infiltration and key area lighting schedules are also important factors to consider. Previously, it is believed that performance gap is mainly due to an over-estimated convection coefficient (mainly external), but results indicate that external convection coefficient would not stronly affect the energy demand results, unless there is a huge error exists in the assumption.\\

    Finally, as outdoor environment largely affects the simulation results, global warming effect and heat island effect are also considered in this thesis.\\
